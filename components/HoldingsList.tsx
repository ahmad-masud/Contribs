...
export default function HoldingsList({ items, netContributed = 0, cashBalance = 0, onRemove, onValueChange }: { items: Holding[]; netContributed?: number; cashBalance?: number; onRemove?: (id: string) => void; onValueChange?: (totalValue: number) => void }) {
  ...
  // Report holdings-only value to parent so it can combine with cash once
  useEffect(() => { onValueChange?.(holdingsValue); }, [onValueChange, holdingsValue]);
